...
def wine_steam_download(progress_id, appid, name, image, message, login, password, folder):
    ...
    while os.path.isfile(message_file_path):
        ...
        time.sleep(1)
        float_downloaded = float(downloaded.replace(",","."))
        float_todownload = float(todownload.replace(",","."))
        if percent == "100":
            break
...
def parse_latest_message(latest_message, install_method):
    ...
    matches_finished    = re.findall(pattern_finished,latest_message,re.DOTALL)
...
def start_wine_steam_download(appid, login, password, folder):
    ...
    wine_executable     = VdfAppFile.find(appid + ".config.launch.0.executable")
...
def create_script(progress_id, name, image, message, systemtype):
    ...
    target_filename = "rom_" + progress_id + "." + fileending
    wine_name   = get_wine_name(name)
    
    filecontent = get_script_content(systemtype, progress_id,target_path,target_filename, wine_name)
...
# does some formatting on the common name so it can be used easier for pol-installs
def get_wine_name(name):
    wine_name = str(name)
...
def get_startcommand_by_systemtype(systemtype, target_path, target_filename, wine_name=""):
    switcher = {
        ...
        'wine_steam'            : '/usr/share/playonlinux/playonlinux --run "' + wine_name + '" &',
    }
...
# return pol settings if needed
def get_script_pol_settings(systemtype):
    switcher = {
        'wine_steam' : 'export POL_IgnoreWineErrors=True',
    }
...
def get_script_content(systemtype, progress_id,target_path,target_filename, wine_name=""):
    content  = "#!/bin/sh\n"
    ...
    content += get_script_pol_settings(systemtype)
    content += get_startcommand_by_systemtype(systemtype, target_path, target_filename, wine_name) + "\n"
...
def get_process_name_by_systemtype(progress_id, systemtype):
    ...
    if systemtype == 'psx':
        ...
        content += "mupen64plus"
    elif systemtype == 'wine_steam':
        content += wine_executable
        
    return content

# creates the needed pol prefix to be run with wined steam
def wine_steam_create_prefix(appid, name):
    wine_name = get_wine_name(name)
    pol_installscript_path = download_steam_pol_installscript(appid)
    start_pol_installscript(pol_installscript_path)


# starts the installscript which creates the prefix and the shortcut
def start_pol_installscript(pol_installscript_path):
    cmd = 'gateos-pol ' + pol_installscript_path
    p = subprocess.Popen(cmd, shell=True, close_fds=True)
    p.communicate()


# downloads and returns path of pol installscript
def download_steam_pol_installscript(appid):
    source = 'http://www.gate4games.com/pol/scripts/' + appid + '.sh'
    target_path = FOLDER_DOWNLOADS + "/polscript_" + appid + ".sh"
    download = SmartDL(source, target_path, progress_bar=False)
    download.start(blocking=False)
    while not download.isFinished():
        time.sleep(1)
...
if downloadtype == 'wine_steam':
    message = "Building wineprefix for " + name
    wine_steam_create_prefix(appid, name)
    message = "Downloading " + name
